...
def del_repes(lis: list) -> list:
    lista: list = lis
    for elem in lista:
        for i in range(1, lista.count(elem)):
            lista.pop(lista.index(elem))
    return lista
...
def elementos_exclusivos(s: list, t: list) -> list:
    res: list = []
    for elem in s:
        if (elem not in s and elem in t) or (elem in s and elem not in t):
            res.append(elem)
    for elem in t:
        if (elem not in s and elem in t) or (elem in s and elem not in t):
            res.append(elem)
